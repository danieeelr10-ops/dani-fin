...
function QuickCapture({ open, onClose }) {
  ...
  const ayerStr  = () => { const d = new Date(); d.setDate(d.getDate() - 1); return d.toLocaleDateString('en-CA'); };
...
const ALL_TABS = [
  ...
  {
    path: '/finanzas', featureKey: 'finanzas', label: 'Finanzas',
    icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
      <line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/>
    </svg>,
  },
  {
    path: '/ritual', featureKey: 'ritual', label: 'Ritual',
    icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
  ...
  {
    path: '/planificador', featureKey: 'planificador', label: 'Planificador',
    icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
      <rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/>
      <line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/>
    </svg>,
  ...
  {
    path: '/habitos', featureKey: 'habitos', label: 'Hábitos',
    icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
      <path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/>
    </svg>,
  },
  {
    path: '/metas-vida', featureKey: 'metasVida', label: 'Metas',
    icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
...
export default function Layout({ children }) {
  ...
  const tabFinanzas  = ALL_TABS.find(t => t.path === '/finanzas');
  const tabHabitos   = ALL_TABS.find(t => t.path === '/habitos');

  // Grupos del drawer "Más" mobile — filtrado por features
  const DRAWER_GROUPS_DEF = [
    { label: 'Organización', items: ['/ritual', '/planificador', '/metas-vida', '/rueda-vida', '/notas'] },
  ...
  if (isMobile) {
    return (
      <Box sx={{ height: '100dvh', display: 'flex', flexDirection: 'column', bgcolor: 'background.default' }}>
          ...
          <Box sx={{ flexShrink: 0, px: 1.5, pt: 1, pb: 0.5 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, px: 1.5, py: 1, borderRadius: 2, bgcolor: alpha('#6366f1', 0.1), border: '1px solid', borderColor: alpha('#6366f1', 0.25) }}>
              ...
              <Box sx={{ flex: 1, minWidth: 0 }}>
                <Typography sx={{ fontSize: 12, fontWeight: 700, lineHeight: 1.2 }}>Instalar Rumbo</Typography>
                {isIOS
        ...
        {/* Bottom nav — 5 ítems: Inicio | Finanzas | [+] | Hábitos | Más */}
        <Box sx={{
          flexShrink: 0,
          ...
          {/* Finanzas */}
          <NavTab tab={tabFinanzas} active={pathname.startsWith('/finanzas')} />
          ...
          {/* Hábitos */}
          <NavTab tab={tabHabitos} active={pathname === '/habitos'} />
      ...
      <Box sx={{
        ...
        <Box sx={{ height: 56, display: 'flex', alignItems: 'center', px: collapsed ? 0 : 2, justifyContent: collapsed ? 'center' : 'space-between', flexShrink: 0 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            {!collapsed && <Typography sx={{ fontSize: 15, fontWeight: 700, letterSpacing: '-0.3px' }}>Rumbo</Typography>}
          </Box>
          {!collapsed && (
